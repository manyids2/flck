...
static size_t kitty_send_rgba(char cmd, uint32_t id, uint32_t compression,
...
static void kitty_hide_cursor() { puts("\x1B[?25l"); }

static void kitty_show_cursor() { puts("\x1B[?25h"); }
 ...
 */
static void kitty_flip_buffer_y(uint32_t *buffer, uint32_t width,
                                uint32_t height) {
...
static void kitty_key_callback(key_cb cb) { *_get_key_callback() = cb; }
